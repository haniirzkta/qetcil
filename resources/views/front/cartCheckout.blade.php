                ...
                <div class="card mb-4">
                    <div class="card-body">
                        ...
                        <div class="d-flex align-items-start mb-3">
                            ...
                            <div>
                                <p class="mb-1">{{ $item->bouquet->name }}</p>
                                <p class="text-muted mb-0">{{ $item->quantity }} x
                                    Rp{{ number_format($item->bouquet->price, 0, ',', '.') }}</p>
                                @if ($item->size != 0)
                    ...
                    <div class="card-body">
                        ...
                        <ul class="list-group mb-3">
                            <li class="list-group-item d-flex justify-content-between">
                                <span>Total Harga ({{ $cartItems->sum('quantity') }} Bouquet)</span>
                                <span>Rp{{ number_format($cartItems->sum(function ($item) { return $item->bouquet->price * $item->quantity; }), 0, ',', '.') }}</span>
                            </li>
                        </ul>
                        <div class="d-flex justify-content-between mb-3">
                            <strong>Total Tagihan</strong>
                            <strong>Rp{{ number_format($cartItems->sum(function ($item) { return $item->bouquet->price * $item->quantity; }), 0, ',', '.') }}</strong>
                        </div>
